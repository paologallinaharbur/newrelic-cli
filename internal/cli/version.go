...
const PrereleaseEnvironmentMessageFormat string = `
...
const OldVersionMessage string = `
...
func IsLatestVersion(ctx context.Context, currentVersion string) (bool, error) {
	...
	latestRelease, err := FetchLatestTag(ctx)
	if err != nil {
...
func FetchLatestTag(ctx context.Context) (*GitHubRepositoryTagResponse, error) {
...
func UpgradeCLIVersion(ctx context.Context, internalVersion string, targetVersion string, forceUpgrade bool) error {
	...
	if isDevEnvironment && !forceUpgrade {
		return fmt.Errorf(PrereleaseEnvironmentMessageFormat, internalVersion)
	}
	...
	latestVersion, err := semver.NewVersion(latestTag.TagName)
	if err != nil {
		return fmt.Errorf("error parsing latest CLI version %s", latestTag.TagName)
	}

	if currentVersion.LessThan(latestVersion) {
		output.Printf(OldVersionMessage, currentVersion.String(), latestVersion.String())
	}
